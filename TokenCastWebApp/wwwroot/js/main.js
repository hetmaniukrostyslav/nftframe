...
async function connectDeviceStatusesWS() {
    ...
    deviceStatusesSocket.onmessage = (event) => {
        ...
        if (data.event === 'Online') {
            if (app.deviceStatuses.indexOf(data.deviceId) === -1) {
                app.deviceStatuses.push(data.deviceId)
            }
        }

        if (data.event === 'Offline') {
            const index = app.deviceStatuses.indexOf(data.deviceId)
            if (index !== -1) {
                app.deviceStatuses.splice(index, 1)
            }
        }
    }
}
